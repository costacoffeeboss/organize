...
export default function GoalQuiz({ visible, onClose, onPick, onStartBlank }) {
  ...
  // Top two interest areas → four tailored ideas.
  const ranked = Object.entries(scores).sort((a, b) => b[1] - a[1]).map(([c]) => c);
  const suggestions = ranked.slice(0, 2).flatMap((c) => IDEAS[c] || []);
          ...
          <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginTop: 20 }}>
              {suggestions.map((idea, i) => (
                <Rise key={idea.title} delay={260 + i * 110}>
                  <TouchableOpacity
                    style={styles.idea}
...
const styles = StyleSheet.create({
  ...
  idea: {
    backgroundColor: COLORS.panel, borderWidth: 1, borderColor: 'rgba(184,135,75,0.45)',
